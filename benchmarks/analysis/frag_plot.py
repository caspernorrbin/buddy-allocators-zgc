...
def main():
    PROGRAM = "heap"
    binary_allocs, binary_fails = parse_input_file(f"./collections/{PROGRAM}_binary.txt")
    bt_allocs, bt_fails = parse_input_file(f"./collections/{PROGRAM}_bt.txt")
    bt_alt_allocs, bt_fails_alt = parse_input_file(f"./collections/{PROGRAM}_bt2.txt")
    ibuddy_allocs, ibuddy_fails = parse_input_file(f"./collections/{PROGRAM}_ibuddy.txt")


    binary_buddy = transform_data(binary_allocs)
    bt_buddy = transform_data(bt_allocs)
    bt_buddy_alt = transform_data(bt_alt_allocs)
    ...
    y4 = [np.array([p[3][i] for p in bt_buddy_alt]) for i in range(5)]
    DOT_SIZE = 3
    XLIM = 200
    YLIM = 1500000
    ...
    # print(binary_fails)
    ax01 = ax0.twinx()
    ax01.scatter(x1, binary_fails, label="Failed allocs", color="black", s=DOT_SIZE)
    ...
    # Set y-axis titles
    ax0.set_ylabel("Unused memory (B)")
    ax0.set_xlabel("Failed allocations")
    ax01.set_ylabel("Failed allocation size (B)")
    ...
    ax11 = ax1.twinx()
    ax11.scatter(x2, bt_fails, label="Failed allocs", color="black", s=DOT_SIZE)
    ...
    ax1.set_ylabel("Unused memory (B)")
    ax1.set_xlabel("Failed allocations")
    ax11.set_ylabel("Failed allocation size (B)")

    ax21 = ax2.twinx()
    ax21.scatter(x3, ibuddy_fails, label="Failed allocs", color="black", s=DOT_SIZE)
    ...
    ax2.set_ylabel("Unused memory (B)")
    ax2.set_xlabel("Failed allocations")
    ax21.set_ylabel("Failed allocation size (B)")
    ...
    ax31 = ax3.twinx()
    ax31.scatter(x4, bt_fails_alt, label="Failed allocs", color="black", s=DOT_SIZE)
    ax3.bar(x4,y4[0], 1, color="r", label="<=16")
    ax3.bar(x4,y4[1], 1, bottom=y4[0], color="tab:blue", label="<=32")
    ax3.bar(x4,y4[2], 1, bottom=y4[0]+y4[1], color="b", label="<=128")
    ax3.bar(x4,y4[3], 1, bottom=y4[0]+y4[1]+y4[2], color="y", label="<=1024")
    ax3.bar(x4,y4[4], 1, bottom=y4[0]+y4[1]+y4[2]+y4[3], color="m", label="<=8192")
    ax3.bar(x4,y4[4], 1, bottom=y4[0]+y4[1]+y4[2]+y4[3]+y4[4], color="c", label=">8192")
    ax3.ticklabel_format(style='plain')
    ax3.legend()
    ax3.set_title("Binary tree buddy (alt)")
    ax3.set_ylim(0, YLIM)
    ax3.set_xlim(0, XLIM)

    ax3.set_ylabel("Unused memory (B)")
    ax3.set_xlabel("Failed allocations")
    ax31.set_ylabel("Failed allocation size (B)")
